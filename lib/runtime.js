...
Runtime.prototype.start = function (appName, callback) {
  ...
  this.childProcess.on('close', function (code, signal) {
    console.log('runtime closed', code);
  ...
  if (appName) {
    this._connectDebuggerServer(function () {
      client.getWebapps(function (err, webapps) {
        ...
        console.log('launching', 'app://' + appName + '.gaiamobile.org/manifest.webapp');
        webapps.launch('app://' + appName + '.gaiamobile.org/manifest.webapp', function (err) {
          if (err) {
            console.log(err);
            return;
          }
...
Runtime.prototype.reopen = function (appName) {
  this.stop();
  this.start(appName);
};
...
Runtime.prototype._connectDebuggerServer = function (callback, retryInterval) {
  console.log('finding ports... \n');

  retryInterval = retryInterval || 100;
  ...
  findPorts({detailed: true}).then(function (results) {
    ...
    if (!isPortOpened) {
      console.log('debugger server at port 6000 not found\n');
      setTimeout(function () {
        self._connectDebuggerServer(callback);
      }, retryInterval);
